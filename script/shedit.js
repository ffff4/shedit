...
function utf8_decode(utftext) {
        ...
        var c = c1 = c2 = 0;
        var cod1 = 0;
        var Alfavit = 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ';
        while ( i < utftext.length ) {
            ...
            if (c < 128) {
               ...
               // console.log(' cod1 '+cod1,' '+String.fromCharCode(((c & 31) << 6) | (c2 & 63)));  
                if  (cod1 === 1168){
                  string += String.fromCharCode(1040);//А
                }
                else
                if  (cod1 === 9240   ){
                  string += String.fromCharCode(1041);//Б
                   console.log('Б---');
                }
                else
                if  (cod1 === 9241   ){
                  string += String.fromCharCode(1042);//В
                }
                else
                if  (cod1 === 9244   ){
                  string += String.fromCharCode(1043);//Г
                }
                else
              if  (cod1 === 9245   ){
                  string += String.fromCharCode(1044);//Д
                }
                else
                if  (cod1 === 9250        ){
                  string += String.fromCharCode(1045);//Е
                }
                else
                  if  (cod1 === 1153     ){
                  string += String.fromCharCode(1025);//Ё
                }
                else
                  if  (cod1 === 9235         ){
...
$.post("save.php?"+Name_sheme_l, encodeURIComponent(canvas.toSVG()) );
 ...
 if (poz1>0)
   ...
   console.log('tx_sg - ',tx_sg);
    ...
    obj.setTransformMatrix(transformMatrix1);
       ...
       //obj.setLeft(transformMatrix2[4]/2);//m_x/2 оказывается одном и том же месте
...
///alert('Новое SVG-изображение будет в библиотеке после закрытия/открытия окна')

}; 

function Set_New_Zoom(thelist)
{
 var idx = thelist.selectedIndex;
 var val = thelist.options[idx].innerHTML;
canvas.setZoom(val);
